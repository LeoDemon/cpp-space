...
int TestTemp3();
int TestMap();
extern "C"int TestVector(const char *puIn);
int TestList();
...
int TestHashTable();
extern "C" int TestBuffer();
int TestVariable();
int TestStl();
int TestStl2();
extern "C" int TestStl3();
extern "C" int TestExcel();
...
extern "C" int TestQryfolder(const char path[]);
extern "C" int TestPointer();
extern "C" int DoBalaData(char*);
...
{
	public:
		CstrPos(int pos, const char * str)
		{
			m_pos = pos;
			m_str = str;
		}
		...
		//greater 重载 > 
		bool operator > (const CstrPos& r) const
		{
			cout<<"++++++this->m_str=="<<this->m_str<<endl;
			cout<<"++++++r.m_str=="<<r.m_str<<endl;
			return m_str > r.m_str;//降序排列:入参类中属性值大于当前类中属性值
...
{
	public:
		DataBaseLab(string sLab, string sDb)
		{
			this->sLab = sLab;
			this->sDb = sDb;
		}
